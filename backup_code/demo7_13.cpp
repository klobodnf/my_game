...
if (bitmap->bitmapinfoheader.biBitCount == 8)
   {
	// 是8位、证明位图数据是存在调色版的、所以读取调色版数、基本上、现在能用得上调色版的
	// 只能是8位的256色的、1、2、4位都已经落后了、16位、24也用不着调色版、所以长度基本上就是已经
	// 固定了
...
// flip the bitmap
// flip the bitmap
// 因为大多数位图的数据文件总是上下颠倒的、所以总要用一下翻转位图函数
// 这里的第二个参数、也就是每行的字节数、如果是8位色、一个像素为一个字节、则每行的字节数刚好等于宽度、
// 如果是24位、则为一个像素为三字节、所以等于宽度的3倍、于是有了下面的参数
...
for (index=0; index<num_rects; index++)
	...
	// 如果目标矩形的左边界比裁剪边界的左边界靠左、那么裁剪边界等于矩形边界
    if (clip_list[index].left < region_data->rdh.rcBound.left)
       region_data->rdh.rcBound.left = clip_list[index].left;
	
	// 如果目标矩形的右边界比裁剪边界的右边界靠右、那么裁剪边界等于矩形边界
    if (clip_list[index].right > region_data->rdh.rcBound.right)
       region_data->rdh.rcBound.right = clip_list[index].right;

	// 如果目标矩形的上边界比裁剪边界的上边界靠上、那么裁剪边界等于矩形边界
...
// 基本上、region_date只是为了描述剪切版列表而已、用完了就删除吧
free(region_data);
return(lpddclipper);
...
// 必须要持有背景表面而不是直接拷贝去缓冲表面、
// 因为在每一帧中、缓冲表面都要被异形的移动而不断重画、
// 所以总是第一次拷贝整个背景去缓冲表面、然后再用异形的表面
// 去覆盖相应位置的缓冲表面、所以表现得异形就在背景上不断行走一样、
// 其实两个画面都是在不断重画中的
